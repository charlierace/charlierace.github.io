...
RSS_PAGES_TO_FETCH = 20  # each feed page is roughly 10-20 items; deeper history
                          # is needed since auctions close ~7 days after listing,
                          # so "closing soon" items sit near the OLD end of the feed
    ...
    for page in range(1, RSS_PAGES_TO_FETCH + 1):
        ...
        for item in page_items:
            ...
            if link and link not in seen_links:
                seen_links.add(link)
                items.append({"link": link, "title": title, "pubDate": pub_date})
...
def main():
    ...
    pub_dates = [item["pubDate"] for item in rss_items if item.get("pubDate")]
    diagnostics["oldest_pub_date"] = pub_dates[-1] if pub_dates else "unknown"
    diagnostics["newest_pub_date"] = pub_dates[0] if pub_dates else "unknown"
...
def write_results(matches, diagnostics, error=None):
    ...
    with open("results.md", "w") as f:
        ...
        f.write(f"- RSS date range covered: {diagnostics.get('oldest_pub_date', 'unknown')} to {diagnostics.get('newest_pub_date', 'unknown')}\n")
