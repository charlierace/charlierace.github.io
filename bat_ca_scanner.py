...
that appear to be located/registered in California, and builds a data
table (year, make, model, current bid, time left, title/reg place,
location) for each match.
...
find California + closing-soon candidates.

STAGE 2 (1 request per match, only for the narrowed-down candidates):
visit each matched listing's own page to pull the human-readable
location, current bid, and any title/registration-place mention. These
fields aren't in the Stage 1 index, so this step is unavoidable — but
it's now applied to a handful of matches, not all 1,301 listings.
...
back empty. If either looks consistently wrong/empty in results.json,
send me a listing URL and I'll tune the extraction.
"""
...
def fetch_listing_details(url):
    ...
    loc_m = re.search(
        r"Location:\s*([A-Za-z .,'-]+?\d{0,5})(?:\s{2,}|\||Chassis|VIN|$)", plain
    )
    if loc_m:
        details["location"] = loc_m.group(1).strip()

    title_place_patterns = [
        r"title(?:d)?\s+(?:is\s+)?in\s+([A-Za-z ]+?)(?:\.|,|\s{2,})",
        r"registered\s+in\s+([A-Za-z ]+?)(?:\.|,|\s{2,})",
    ]
    for pat in title_place_patterns:
        m = re.search(pat, plain, re.IGNORECASE)
        if m:
            details["title_place"] = m.group(1).strip()
            break
...
def main():
    ...
    print(f"{len(candidates)} California candidates within window — fetching details...\n")
    ...
    for i, (listing, end_time) in enumerate(candidates, 1):
        ...
        matches.append({
            ...
            "location": details["location"] or f"lat={listing.get('lat')}, lon={listing.get('lon')}",
            "url": url,
        })

        time.sleep(1)  # be polite — this now only runs for the narrowed match set
...
def write_results(matches, diagnostics, error=None):
    ...
    with open("results.md", "w") as f:
        ...
        elif diagnostics.get("ca_matches", 0) > 0 and diagnostics.get("bid_found", 0) == 0:
            f.write(
                "\n**Note:** no current bid values were found on any matched "
                "listing page. The bid-extraction patterns likely need "
                "tuning — send me one listing URL from the table above and "
                "I'll fix it.\n"
            )
