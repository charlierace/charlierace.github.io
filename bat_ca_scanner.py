...
def main():
    ...
    try:
        ...
        diagnostics["auctions_page_status"] = resp.status_code
        resp.raise_for_status()
        soup = BeautifulSoup(resp.text, "html.parser")
    except requests.RequestException as e:
        print(f"Could not reach BaT: {e}")
        write_results([], diagnostics, error=str(e))
        sys.exit(1)

    listing_links = find_listing_links(soup)
    diagnostics["links_found"] = len(listing_links)
...
def write_results(matches, diagnostics, error=None):
    ...
    with open("results.md", "w") as f:
        ...
        f.write("---\n\n")
        f.write("### Diagnostics\n\n")
        f.write(f"- Auctions page HTTP status: {diagnostics.get('auctions_page_status')}\n")
        f.write(f"- Listing links found on auctions page: {diagnostics.get('links_found', 0)}\n")
        ...
        if diagnostics.get("links_found", 0) == 0:
            f.write(
                "\n**Likely cause:** 0 listing links were found on the auctions "
                "page at all. This usually means BaT is either rendering "
                "listings via JavaScript (invisible to a plain HTTP request) "
                "or blocking non-browser requests. This needs a fix to the "
                "scraping approach, not just the CA-matching logic.\n"
            )
        elif diagnostics.get("checked_within_window", 0) == 0:
            f.write(
                "\n**Likely cause:** listing links were found, but the end-time "
                "parser (`extract_end_time`) never found a usable end time. "
                "The HTML structure BaT uses likely differs from what the "
                "selectors expect and needs updating.\n"
            )
